...
if(mode==='--device'){
  const wrapper=resolve(root,'artifacts/build-spec/device-branding.json');
  mkdirSync(dirname(wrapper),{recursive:true});
  writeFileSync(wrapper,JSON.stringify({include:[resolve(root,spec),resolve(root,'apps/RayNeoCompanion/project-branding.yml')]},null,2)+'\n');
  spec=wrapper;
}
const generated=spawnSync('xcodegen',['generate','--spec',spec,'--project-root',resolve(root,'apps/RayNeoCompanion'),'--project',resolve(root,'apps/RayNeoCompanion')],{cwd:root,stdio:'inherit'});
