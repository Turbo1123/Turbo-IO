...
1. 将 AppUI / AppPackage / AppTransport / AppBridge 的 `.m/.h`、`AppGallery.inc` 与 `app.c/app.h/app_command.h` 及其包含的协议头加入现有 iOS 插件目标。沿用宿主已有 `ProtocolContext`、`ExperimentalOTAFlash`、`ResearchUI`；接口头可在 `official-addon/focus-edition/` 找到。链接 Foundation、UIKit、UniformTypeIdentifiers、Security、zlib。不要把新门禁函数改为常量“允许”。
